...
const Signup: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Remove explicit alert, UI will navigate to verification page
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      try {
        // Register user in backend (MongoDB) if they don't exist
        await authAPI.register({
          name: result.user.displayName || 'Google User',
          email: result.user.email || '',
          password: result.user.uid, // use UID as backend password
          phone: result.user.phoneNumber || '0000000000',
          role: 'passenger'
        });
      } catch (backendError: any) {
        // If user already exists in backend, it will throw an error, which we can ignore for Google sign in
        // since they might be trying to log in but clicked sign up with google
      }
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center p-4">
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          ...
          <CardTitle className="text-2xl font-bold text-gray-900">Join RailReserve</CardTitle>
